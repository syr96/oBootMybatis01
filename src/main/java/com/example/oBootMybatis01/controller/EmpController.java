...
public class EmpController {
	...
	public String empList(Emp emp, String currentPage, Model model) {
		...
		// Emp 전체 Count
		int totalEmp = es.totalEmp();	// 21 나옴
		
		// Paging 작업				21			0
		Paging page = new Paging(totalEmp, currentPage);
		// Parameter emp --> Page만 추가 Setting 
		// 페이징 작업하려고 emp테이블에 start와 end변수를 세팅해놓음
		...
		// Model에 메소드 수행한 결과 넣음
		model.addAttribute("totalEmp", totalEmp);	// int형 숫자 (emp 테이블 총원 의미)
		model.addAttribute("listEmp", listEmp);		// 실제 사원객체 들의 값을 10개단위로 list형태로 가지고있음
	...
	public String listSearch3(Emp emp, String currentPage, Model model) {
		...
		int totalEmp = es.condTotalEmp(emp); // 조건(cond)에 따른 TotalCount 만듬
		// Paging 작업				23			0
		Paging page = new Paging(totalEmp, currentPage);
		// Parameter emp --> Page만 추가 Setting
		emp.setStart(page.getStart());		// 시작시 1
		emp.setEnd(page.getEnd());			// 시작시 10
		System.out.println("emp.getStart --> " + emp.getStart());
		System.out.println("emp.getEnd --> " + emp.getEnd());
		
		// Model에 메소드 수행한 결과 넣음
		model.addAttribute("totalEmp", totalEmp);
		List<Emp> listEmp = es.listSearchEmp(emp);
		System.out.println("EmpController listSearch3 listSearchEmp.size()=>" + listEmp.size());
		model.addAttribute("totalEmp", totalEmp);
	...
	public String detailEmp(int empno, Model model) { // 파라미터가 한개일경우 Dto선언 안하고 empno 로 받을 때도 있음
...
//		emp = session.selectOne("tkEmpSelOne",    empno);
	...
	public String updateFormEmp(int empno, Model model ) {
		...
		// 3.해결책  : 년월일만 짤라 넣어 주어야 함(문자열로 넣어줌)
		String hiredate = "";
		if (emp.getHiredate() != null) {
			hiredate = emp.getHiredate().substring(0,10);	// substring(0,10)에 의해 date를 잘라서 string 형으로 만듬
...
//   	updateCount = session.update("tkEmpUpdate",emp);
		...
		System.out.println("EmpController updateEmp result --> " + updateCount);

		model.addAttribute("uptCnt",updateCount);	//Test Controller간 Data 전달
		model.addAttribute("kk3","Message Test");	//Test Controller간 Data 전달 redirect와 forward 구분하기 위해 그냥 넣은거 특별한 의미 없음
		
		// listemp와 forward 차이
		// forward는 model 가지고감 redirect는 안가지고감
	...
	public String mailTransport(HttpServletRequest request, Model model) {
		...
		String tomail = "bigwise92@naver.com";		// 받는사람
		System.out.println(tomail);
		String setfrom = "chtaehyunl@gmail.com";
	...
	public String writeDeptCursor(Model model) {
		...
		for(Dept dept : deptLists) { // 이해용으로 뿌려준거지 실제쓰는 코드는 아님 start... 출력문 같은느낌
	...
	public String interCeptor(String id, Model model) {
		...
		return "interCeptor";	// User 존재하면 User 이름 조회 Page +가짜 viewResolver 실제 수행되진않고 폼으로 씀(이거안쓰면 return 값없어서 에러나므로)
	}
